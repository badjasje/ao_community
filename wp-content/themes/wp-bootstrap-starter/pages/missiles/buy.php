		...
		<div class="row unitRow" style="background-color: rgba(<?php echo $buyBackColor;?>, <?php echo 0.6-($count/25);?>);">
			...
			<div class="col-md-2 celBlock owned">
				...
				<span class="columnDataRight">
					...
					(<span id="<?php echo $key;?>_ordered"><?php echo $units_ordered; ?></span>)
				</span>
			</div>
			...
			<div class="col-md-1 celBlock max">
				...
				<span class="columnDataRight">
					...
					?>
					<span id="<?php echo $key;?>" class="allbutton" data-nw="<?php echo $order['networth'];?>"
						data-price="<?php echo $order['price'];?>" data-key="<?php echo $key;?>"
						data-amount="<?php echo (min($max_money, $max_turns, $max_space));?>">
						<?php echo (min($max_money, $max_turns, $max_space));?>
					</span>
				</span>
			</div>
			...
			<div class="col-md-3 celBlock inputBlock">
				...
				<?php else:?>
					<input class="unitInput buyInput  buy_<?php echo $key;?>"
						data-nw="<?php echo $order['networth'];?>" data-key="<?php echo $key;?>"
						data-price="<?php echo $order['price'];?>" type="number" id="<?php echo $key;?>" min="0"
						name="<?php echo $key;?>"
						style="border: solid rgba(<?php echo $buyBackColor;?>, <?php echo 0.6-($count/25);?>);border-width:5px 13px 5px 13px;"/>
				<?php endif;?>
			</div>
		</div> <!-- // Close Unit row -->
		<?php
