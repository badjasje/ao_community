...
foreach ($bonus_posts as $bonus) {
	...
	if($used != 'yes'){ // Check if bonus is used
		...
		?>
		<div class="blockHeader">You can now receive a clan bonus of $ <?php echo number_format($money, 0, ',', ' '); ?> and <?php echo $turns;?> turns.</div>
		<div class="blockHeader spaceNotice">Auto receive in <?php echo human_time_diff( $time,$timestamp);?></div>
		<a class="mainSubmit" href="/receive_bonus.php/?id=<?php echo $event_ID;?>"
			onclick="return confirm('Are you sure you want to receive your bonus?')">Receive Bonus</a>
		<div class="pageSpacer"></div>
		<?php
	}
} // End clan bonus check ?>