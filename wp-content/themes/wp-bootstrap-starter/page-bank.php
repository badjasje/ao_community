...
<div class="row pageRow">
	<div class="fw-row">
		<form id="bank">
			...
			<div class="blockHeader" style="border-bottom:0px;">
				Maximum of 10 deposits. $ <?php echo number_format($max_dep, 0, ',', ' '); ?> maximum per deposit.
			</div>
	...
	(function($) {
			...
			if(confirm("Are you sure? This deposit will return $ "+bankvalue)) {
				...
				withdraw.done(function (response, textStatus, jqXHR){
					...
					if(array.next == true){
						...
						$("#amount").attr({"max" : array.newmaxdep,"min" : 0});
						$("#maxdep").attr({"data-max" : array.newmaxdep,"min" : 0});
						var depval = 0;
						var inclinterest = 0;
						var unlocked = 0;
		...
		$('form').submit(function( event ) {
			...
			request = $.ajax({url: "/bank_money.php",type: "post",data: serializedData});
