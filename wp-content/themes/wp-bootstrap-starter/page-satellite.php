...
<div class="row pageRow">
	...
	<div class="fw-row <?php echo $disableClass;?> satblock">
		...
		elseif($sat_progress != '0') {
			...
			<?php
			$args = ['posts_per_page' => -1,'meta_key' => 'user_placed_id','meta_value' => $userId,'post_type' => 'market_order',];
			...
			foreach ($orders as $order){
				...
				if($order_type != 'satellite') continue;
	...
	<script>
		(function($) {
			...
			$( "body" ).on('submit','#cancelsat',function(cancelevent) {
				...
				cancel = $.ajax({url: "/cancel_order.php",type: "post",data: serializedcancelData});
			...
			$(document).on('submit','#satbuild',function(event){
				...
				request = $.ajax({url: "/satellite.php",type: "post",data: serializedData});
