 ...
 */
 ...
 function get_avatar_url2($get_avatar){
...
$admin_users = array(2,10,1);
if(!in_array(get_current_user_id(), $admin_users)){
...
function twentytwelve_setup() {
	...
	register_nav_menu( 'primary_loggedout', __( 'Primary Logged out Menu', 'twentytwelve' ) );
	/*
...
foreach($units as $key => $unit){
	...
	$unit_networth+= $units_owned[0]*$unit['price']*($unit['networth']/100);
...
$land_networth = get_user_meta($user_ID, 'land');
$land_networth = $land_networth[0]*0.85;
update_user_meta( $user_ID,'networth',$research_NW+$building_networth+$unit_networth+$land_networth+$missile_networth);
...
function your_function() {
	$timestamp = strtotime(date('Y-m-d H:i:s'));
update_user_meta( $user_ID,'last_online',$timestamp-8000);
}
