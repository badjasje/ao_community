...
$defender_unit_losses = calculate_unit_kills($defender_unit_array, $attacker_type_damage, $attack_type);
...
$attacker_unit_losses = calculate_unit_kills($attacker_unit_array, $defense_attack_type, 'defend');
...
if($war_type != 'none' && $result == 'success') {
	...
	if($def_total_units != 0 && $defender_units_lost != 0){
		$unit_points = $defender_units_lost/$def_total_units;
	}
	...
	if ($killed != true) {
		...
		if($war_type == 'mutual') {
		$clan_points = $POINTS_KILL_MUTUAL;
		}
		elseif($war_type == 'incoming') {
			$clan_points = $POINTS_KILL_INCOMING;
		}
		elseif($war_type == 'outgoing') {
			$clan_points = $POINTS_KILL_OUTGOING;
		}
	}
	...
	/* add ponts */
	$starting_points = get_post_meta($attack_clan_id,'clan_points')[0];
