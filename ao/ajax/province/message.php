...
function ajax_message($province, $return) {
    ...
    if($message_ID == 'first') {
        ...
        $conv = Conversation::create($province->get('id'), $receiver->get('id'), $title, $message_text);
        $link = Request::siteUrl().'/conversations';
    } else {
        $conv = Conversation::make($message_ID);
        $link = $conv->getLink();
    }
    $conv->addMessage($province->get('id'), $receiver->get('id'), $message_text);
