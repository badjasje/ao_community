...
function ajax_startingbonus($province, $return) {
    ...
    if(!empty($province->getStartingBonus())) return array('status' => 'You already have a startbonus.');
    $bonustype = Request::post('bonustype');
    if(!$province->setStartingBonus($bonustype)) return array('status' => 'No such startbonus.');
