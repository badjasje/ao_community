...
function ajax_reset($province, $return) {
    ...
    if(!empty($reset_status)) return array('status' => 'You have already reset this round');
    ...
    if($clan = $province->getClan()) {
        ...
        if(count($incomingWars) > 0) return array('status' => 'You cannot reset your account while having incoming clan wars');
        $outgoingWars = get_posts(array('numberposts' => -1, 'post_type' => 'wars', 'meta_key' => 'declared_by', 'meta_value' => $clan->get('id')));
        if(count($outgoingWars) > 0) return array('status' => 'You cannot reset your account while having outgoing clan wars');
    }
