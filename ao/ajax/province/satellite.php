...
function ajax_satellite_cancel($province) {
    ...
    if(!$order) return array('status' => 'Order not found');
    $result = $order->cancel();
...
function ajax_satellite_demolish($province) {
    ...
    if(!$satellite) return array('status' => 'No such sat');
    if($satellite['num'] == 0)  return array('status' => 'Sat not owned');
    $demo_cost = round($satellite['price'] * Settings::get('sat_demo_price'));
    if($demo_cost > $province->getMoney()) return array('status' => 'Insufficient funds');
...
function ajax_satellite_order($province) {
    ...
    if(!$sc || $sc['level']==0) return array('status' => 'Research satellite construction you tool');
    if($province->getSatelliteNum() > 0) return array('status' => 'No more then one sat per province');
    if(count($province->getOrderedSatellites()) > 0) return array('status' => 'No more then one sat per province');
    if($province->getTurns() < Settings::get('sat_turn_cost')) return array('status' => 'Not enough turns');
    $sat = Request::post('sat');
    $satellite = $province->getSatellites($sat);
    if(!$satellite) return array('status' => 'No such sat');
    if($satellite['price'] > $province->getMoney()) return array('status' => 'Insufficient funds');
...
function ajax_satellite_activate($province) {
    ...
    if(!$satellite) return array('status' => 'No such sat');
    if($satellite['num'] == 0)  return array('status' => 'Sat not owned');
    if($province->getTurns() < Settings::get('stealthsat_turn_cost')) return array('status' => 'Not enough turns');
    if($province->get('sat_morale') < Settings::get('stealthsat_morale_cost')) return array('status' => 'Not enough satellite power');
